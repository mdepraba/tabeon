...
// expose focus function, so can be called in MinimalView
...
<template>
  <div class="relative w-full rounded-full">
    <form @submit.prevent="emit('submit')" class="relative w-full">
      <span class="absolute left-4 top-1/2 -translate-y-1/2 text-theme-text-muted">
      ...
      <input
        ...
        class="flex h-14 w-full rounded-full border border-theme-border bg-theme-bg/60 px-4 py-2 pl-12 text-base text-theme-text shadow-sm backdrop-blur-md transition-all placeholder:text-theme-text-muted hover:bg-theme-bg/80 focus:border-theme-highlight focus:bg-theme-bg focus:outline-none focus:ring-4 focus:ring-theme-highlight/10"
      />
    </form>

    <!-- Dropdown di DALAM div.relative, LUAR form -->
    <Transition name="fade-slide">
      <div
        v-if="isFocused && suggestions && suggestions.length > 0"
        class="absolute left-0 top-full z-50 mt-2 w-full overflow-hidden rounded-2xl border border-theme-border bg-theme-fg/90 shadow-2xl backdrop-blur-xl">
        <ul class="py-2">
          <li
            ...
            <Search v-if="item.type === 'search'" class="h-4 w-4 flex-shrink-0 opacity-60" />
            <Clock v-else-if="item.type === 'history'" class="h-4 w-4 flex-shrink-0 opacity-60" />
            <Star v-else-if="item.type === 'bookmark'" class="h-4 w-4 flex-shrink-0 opacity-60 text-yellow-500" />
